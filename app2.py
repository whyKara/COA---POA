...
def read_excel_file(file, sheet_name):
    try:
        ...
        for test_id, col in zip(test_ids, test_columns):
            ...
            for col_offset in range(1, 5):
                if col == 3:
                    question_id = sheet.cell(row=19, column=col + col_offset).value
                    question_marks = sheet.cell(row=20, column=col + col_offset).value
                    co_attainment = sheet.cell(row=21, column=col + col_offset).value
                else:
                    question_id = sheet.cell(row=19, column=col + (col_offset - 1)).value
                    question_marks = sheet.cell(row=20, column=col + (col_offset - 1)).value
                    co_attainment = sheet.cell(row=21, column=col + (col_offset - 1)).value
